...
import logging
import time
import config # Necesita acceso a config para defaults
...
# --- Funciones de creación individuales ---
...
def create_vector_database(
    ...
    expected_dimension_metadata: Optional[Union[int, str]] = None, # NUEVO argumento
) -> VectorDBInterface:
    """
    ...
    """
    target_collection_name = collection_name or config.CHROMA_COLLECTION_NAME # Usa default si es None
    logger.info(
    ...
    if expected_dimension_metadata:
        logger.info(f"  Expected dimension metadata for new collection: {expected_dimension_metadata}")

    try:
        # --- Pasa los nuevos argumentos a ChromaVectorDB ---
        db_instance = ChromaVectorDB(
            path=config.CHROMA_DB_PATH,
            collection_name=target_collection_name,
            expected_dimension_metadata=expected_dimension_metadata # Pasa el metadato
        )
        # -----------------------------------------------
        # Verificar inicialización después de crear la instancia
        if not db_instance.is_initialized:
             ...
             raise InitializationError(f"ChromaDB instance created but failed initialization check for collection '{target_collection_name}'. Last error: {last_err}")

        logger.info(f"VectorDatabase instance for collection '{target_collection_name}' obtained successfully.")
        return db_instance
    except InitializationError as e: # Re-lanzar errores de inicialización
        logger.error(f"InitializationError during DB creation for collection '{target_collection_name}': {e}", exc_info=True)
        raise e
    except Exception as e:
        logger.error(
            f"Unexpected error creating VectorDatabase instance for collection '{target_collection_name}': {e}", exc_info=True
        )
...
def get_initialized_components(
    ...
    try:
        # Obtener el vectorizador (puede estar cacheado por Streamlit si se llama desde allí)
        ...
        # Obtener la instancia de BD inicial apuntando a la colección por defecto.
        # No pasamos metadatos esperados aquí, ya que es la instancia "base".
        initial_db = create_vector_database(collection_name=None, expected_dimension_metadata=None)

        end_time = time.time()
        logger.info(
            f"Initial components for model {model_name} ready in {end_time - start_time:.2f} seconds."
        )
        # Devuelve el vectorizador y la instancia de BD inicial
        return vectorizer, initial_db
    except InitializationError as e:
        logger.critical(
            f"Fatal error during component initialization for model {model_name}: {e}", exc_info=True
        )
        raise # Re-lanza el error específico
    except Exception as e:
        logger.critical(
